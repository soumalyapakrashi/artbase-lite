...
session_start();
// if(!isset($_POST["login"]))  session_destroy();  // This is used just to stop automatically logging in everytime site is opened
$login_fail = 0;

if(!isset($_SESSION["logged_in"]))
    $_SESSION["logged_in"] = 0;
elseif($_SESSION["logged_in"] == 1) {
    if(substr($_SESSION["userid"], 0, 1) === "A")
        header("Location: ./artistdetails.php");
    else if(substr($_SESSION["userid"], 0, 1) === "C")
...
if(isset($_POST["login"])) {
    ...
    // Query the database for the required tuple
    $stmt = $pdo->query("SELECT * FROM ACCOUNT WHERE email = \"" . $email . "\"");
    $account = $stmt->fetch(PDO::FETCH_ASSOC);

    // If required entry is not present in the database
    if(empty($account)) {
        $login_fail = 1;
    }
    // If required entry is present, check for the password
    else {
    ...
    <div class="container login">
        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
            <div class="form-group">
                ...
                <input type="email" class="form-control" id="inputemail" name="email" aria-describedby="emailHelp" autocomplete="email">
            </div>
            <div class="form-group">
                <label for="inputpassword">Password</label>
                <input type="password" class="form-control" id="inputpassword" name="password" autocomplete="password">
            </div>
            <button type="submit" class="btn btn-primary btn-block" name="login">Login</button>
            <a role="button" class="btn btn-secondary btn-block" href="./signup.php">Sign Up</a>
        </form>

        <?php
        // If login fails, then print an alert message
        if($login_fail == 1) {
            echo "
            <div class=\"alert alert-danger dialog\" role=\"alert\">
