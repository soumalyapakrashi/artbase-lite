...
// Get the user id of the customer or artist
$uid = $_SESSION["userid"];

// If the user is an artist, fetch the name and the picture from the database
if($uid[0] === "A") {
...
// If the user is a customer, then fetch only the name from the database
else if($uid[0] === "C") {
...
$query = "SELECT ARTWORK.title, ARTWORK.price, ARTWORK.picture, ARTWORK.status, ARTIST.name FROM ARTWORK JOIN ARTIST ON ARTWORK.aid = ARTIST.aid";
$stmt_artworks = $pdo->query($query);
...
<body>
    ...
    <div class="container showcase">
        ...
        <div class="cards">
            ...
            while($artworks = $stmt_artworks->fetch(PDO::FETCH_ASSOC)) {
                echo "<div class=\"card\">";
                echo "<img src='" . $artworks["picture"] . "' alt='" . $artworks["title"] . "' class=\"card-img-top\">";
                echo "<div class=\"card-body\">";
                echo "<h4 class=\"card-title\">" . $artworks["title"] . "</h4>";
                echo "</div>";
                echo "<ul class=\"list-group list-group-flush\">";
                echo "<li class=\"list-group-item\">Artist: " . $artworks["name"] . "</li>";
                echo "<li class=\"list-group-item\">Price: " . (($artworks["price"]) ? "Rs. " . $artworks["price"] : "NA") . "</li>";
                echo "<li class=\"list-group-item\">Status: " . $artworks["status"] . "</li>";
                echo "</ul>";
                echo "<div class=\"card-body\">";
                echo "<a href=\"#\" class=\"card-link\" onclick=\"artworkRequest('" . $artworks["title"] . "')\">Details</a>";
                echo "</div>";
                echo "</div>";
            }
