...
// If the currently logged in user is a customer
if($_SESSION["userid"][0] === "C") {
    $cid = $_SESSION["userid"];

    // Get the details of the currently logged in customer
    $query = "SELECT name, address FROM CUSTOMER WHERE cid = \"" . $cid . "\"";
    $stmt_customerdetails = $pdo->query($query);
    $customer_details = $stmt_customerdetails->fetch(PDO::FETCH_ASSOC);

    // Get details about the transactions made by the customer
    $query = "SELECT CUSTOMER_TRANSACTION.title, CUSTOMER_TRANSACTION.purchase_price, ARTIST.name FROM CUSTOMER_TRANSACTION JOIN ARTWORK ON CUSTOMER_TRANSACTION.title = ARTWORK.title JOIN ARTIST ON ARTWORK.aid = ARTIST.aid WHERE CUSTOMER_TRANSACTION.cid = \"" . $cid . "\"";
    $stmt_transactions = $pdo->query($query);
    $transactions = $stmt_transactions->fetch(PDO::FETCH_ASSOC);
}

// If the user is actually an artist, then redirect to the artistdetails page
...
<body>
    <header>
        <nav>
            ...
            <div class="nav-links">
                ...
                <a href="./customerdetails.php" class="account-info"><?php echo $customer_details["name"]; ?></a>
            </div>
        </nav>
    </header>
    ...
    <section class="transaction-section">
        <div class="info-section">
            <div class="mytable">
                <h1><?php echo $customer_details["name"]; ?></h1>
                <h4>Address</h4>
    ...
    <section class="transaction-section">
        <h2 class="mytable mb-4">Transaction Details</h2>
        <?php
        ...
        if(!$transactions) {
            ...
            echo "<tr onclick=\"artworkRequest('" . $transactions["title"] . "')\">";
            echo "<th scope=\"row\">" . $transactions["title"] . "</th>";
            echo "<td>" . $transactions["name"] . "</td>";
            echo "<td>Rs. " . $transactions["purchase_price"] . "</td>";
            echo "</tr>";

            // This is for the remaining rows if present
            while($responseData = $stmt_transactions->fetch(PDO::FETCH_ASSOC)) {
                echo "<tr onclick=\"artworkRequest('" . $transactions["title"] . "')\">";
                echo "<th scope=\"row\">" . $transactions["title"] . "</th>";
                echo "<td>" . $transactions["name"] . "</td>";
                echo "<td>Rs. " . $transactions["purchase_price"] . "</td>";
